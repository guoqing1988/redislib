...
use Exception;
use Wuhen\Redislib\Commands\Command;
use Wuhen\Redislib\Commands\Factory;
use Wuhen\Redislib\Commands\FactoryInterface;
...
abstract class Model
{
    ...
    {
        $this->initRedisClient($parameters);
        $this->newQuery();
    ...
    protected function initRedisClient($parameters)
    {
        ...
        {
            throw new \Exception(" Redis Exception, msg:".$e->getMessage()." code:".$e->getCode()." line :".__LINE__. " file: ".__FILE__, 1);
        }
    }
    ...
    protected function executeCommand($command)
    {
        ...
        $evalArgsNew = [$evalArgs,$command->getKeysCount()];
        try {
            array_unshift($evalArgsNew, sha1($command->getScript()));
            // echo "111111";
            // var_dump($command->getCommandName());
            // print_r($evalArgsNew);
            // echo "222222\n";
            $data = call_user_func_array([$this->redClient, 'evalsha'], $evalArgsNew);
        } catch (\Exception $e) {
            // print_r($e->getMessage());
            // var_dump(strpos($e->getMessage(), 'NOSCRIPT'));
            // exit;
            if (strpos($e->getMessage(), 'NOSCRIPT') !== false) {
                $evalArgsNew[0] = $command->getScript();
                // var_dump($evalArgsNew);
                $data = call_user_func_array([$this->redClient, 'eval'], $evalArgsNew);
                // var_dump($data);
            } else {
                throw $e;
            }
